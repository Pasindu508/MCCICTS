	...
	$iframe.on("load", function () {
		...
		if (iframe.contentDocument.querySelector("#error #livewebInfo.available")) {
			...
			message.querySelector("a").href =
				"https://web.archive.org/save/https://mccicts.lk/" +
				iframe.src.replace(/.*https:\/\/mccicts\.lk\/?/, "");
		...
		iframe.contentWindow.MCCICTS_AUTH_TOKEN = window.MCCICTS_AUTH_TOKEN;
		iframe.contentWindow.MCCICTS_USER_ROLE = window.MCCICTS_USER_ROLE;
		iframe.contentWindow.MCCICTS_API_BASE = window.MCCICTS_API_BASE;
		if (iframe.contentWindow.MCCICTS?.syncSessionFromParent) {
			iframe.contentWindow.MCCICTS.syncSessionFromParent();
		}
