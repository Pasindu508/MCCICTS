...
function FolderView(folder_path, { asDesktop = false, onStatus, openFolder, openFileOrFolder, onConfigure } = {}) {
	...
	this.getItemFromElement = (el) => {
		if (!el) {
			return null;
		}
		const icon_el = el.closest?.(".desktop-icon") || el;
		return this.items.find((item) => item.element === icon_el) || null;
	};
	...
	if (typeof attachFolderViewContextMenu === "function") {
		attachFolderViewContextMenu(self);
	}
}
